...
if(isset($_POST['rain'],$_POST['temp'],$_POST['ph'],$_POST['soil'])){
	$rain = (double)$_POST['rain'];
	$temp = (double)$_POST['temp'];
	$pH = (double)$_POST['ph'];
	$soil = $_POST['soil'];
//sql query
	$sql = "SELECT * FROM $tableName where 
			lower(soilType) like lower('%$soil%')
			and
			$rain >= minRainfall and $rain <= maxRainfall
			and
			$temp >= minTemp and $temp <= maxTemp
			and
			$pH >= minpH and $pH <= maxpH 
			 ";
	if(!empty($_POST['nutrients'])){
		$addon ='';
		foreach($_POST['nutrients'] as $nutri){
			$addon = $addon.'%'.$nutri;
		}

		$sql = $sql." and nutrients like '$addon%'";
	}
	//echo $sql;
...
<html lang="en">
<head>
		<title>Predictions | Agriculture Data</title>
		<meta charset="utf-8">
